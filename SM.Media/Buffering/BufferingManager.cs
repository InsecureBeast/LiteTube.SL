...
// Type: SM.Media.Buffering.BufferingManager
...
namespace SM.Media.Buffering
{
  public sealed class BufferingManager : IBufferingManager, IDisposable
  {
    ...
    private bool UpdateState()
    {
      ...
      Debug.Assert((timeSpan7 == TimeSpan.MaxValue ? 1 : (timeSpan7 + TimeSpan.FromMilliseconds(500.0) >= TimeSpan.Zero ? 1 : 0)) != 0, string.Format("Invalid timestamp difference: {0} (newest {1} oldest {2} low count {3} high count {4} valid data {5})", (object) timeSpan7, (object) timeSpan1, (object) timeSpan2, (object) num1, (object) num2, (object) (bool) (flag1 ? 1 : 0)));
      ...
      if (this._isBuffering)
      {
        if (flag2)
        {
          ...
          Debug.WriteLine("BufferingManager.UpdateState done buffering (eof): duration {0} size {1} starting {2} memory {3:F} MiB", flag1 ? (object) timeSpan7.ToString() : (object) "none", flag1 ? (object) num3.ToString() : (object) "none", (object) (bool) (this._isStarting ? 1 : 0), (object) ByteConversion.BytesToMiB(GC.GetTotalMemory(false)));
          this.DumpQueues();
      ...
      {
        Debug.WriteLine("BufferingManager.UpdateState start buffering: duration {0} size {1} starting {2} memory {3:F} MiB", (object) timeSpan7, (object) num3, (object) (bool) (this._isStarting ? 1 : 0), (object) ByteConversion.BytesToMiB(GC.GetTotalMemory(false)));
        this.DumpQueues();
      ...
      {
        Debug.WriteLine("BufferingManager.UpdateState read blocking -> {0} duration {1} size {2} memory {3:F} MiB", (object) (bool) (flag3 ? 1 : 0), flag1 ? (object) timeSpan7.ToString() : (object) "none", flag1 ? (object) num3.ToString() : (object) "none", (object) ByteConversion.BytesToMiB(GC.GetTotalMemory(false)));
        this.DumpQueues();
      }
      return flag3;
    }
    ...
    private void UpdateBuffering(TimeSpan timestampDifference, int bufferSize)
    {
      ...
      {
        this._isBuffering = false;
        Debug.WriteLine("BufferingManager.UpdateBuffering done buffering: duration {0} size {1} starting {2} memory {3:F} MiB", (object) timestampDifference, (object) bufferSize, (object) (bool) (this._isStarting ? 1 : 0), (object) ByteConversion.BytesToMiB(GC.GetTotalMemory(false)));
        this.DumpQueues();
      ...
      else
      {
        ...
        if (utcNow - this._bufferStatusTimeUtc >= BufferingManager.BufferStatusUpdatePeriod)
        {
          ...
          Debug.WriteLine("BufferingManager.UpdateBuffering: {0:F2}% duration {1} size {2} starting {3} memory {4:F} MiB", (object) (float) ((double) progress * 100.0), (object) timestampDifference, (object) bufferSize, (object) (bool) (this._isStarting ? 1 : 0), (object) ByteConversion.BytesToMiB(GC.GetTotalMemory(false)));
